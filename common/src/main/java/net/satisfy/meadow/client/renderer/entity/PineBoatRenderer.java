...
public class PineBoatRenderer<T extends PineBoatEntity> extends EntityRenderer<T> {
    private final Map<PineBoatEntity.Type, Pair<ResourceLocation, ListModel<Boat>>> boatResources;

    public PineBoatRenderer(EntityRendererProvider.Context context, boolean hasChest) {
        super(context);
        this.shadowRadius = 0.8f;
        this.boatResources = Stream.of(PineBoatEntity.Type.values()).collect(ImmutableMap.toImmutableMap(type -> type, type ->
                Pair.of(type.getTexture(hasChest), this.createBoatModel(context, type, hasChest))));
    }

    private ListModel<Boat> createBoatModel(EntityRendererProvider.Context context, PineBoatEntity.Type type, boolean hasChest) {
        ModelLayerLocation modelLayerLocation = hasChest ?
                new ModelLayerLocation(new ResourceLocation(Meadow.MOD_ID, type.getChestModelLocation()), "main")
                : new ModelLayerLocation(new ResourceLocation(Meadow.MOD_ID, type.getModelLocation()), "main");
    ...
    @Override
    public @NotNull ResourceLocation getTextureLocation(PineBoatEntity boat) {
